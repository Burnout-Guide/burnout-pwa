...
function updateLanguage(lang) {
  try {
    ...
    document.querySelectorAll('[data-i18n]').forEach(element => {
      ...
      if (!key || !translations || !translations[lang]) return;
    ...
    const crisisSection = document.querySelector('[data-i18n="professional.crisis.title"]');
    if (crisisSection && crisisSection.parentElement) {
      const crisisContent = crisisSection.parentElement.querySelector('ul');
      if (crisisContent) {
        if (lang === 'ru') {
    ...
    const supportSection = document.querySelector('[data-i18n="professional.support.title"]');
    if (supportSection && supportSection.parentElement) {
      const supportList = supportSection.parentElement.querySelector('ul');
      if (supportList && supportList.lastElementChild) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (langToggle) {
    ...
    langToggle.addEventListener('touchend', (e) => {
      ...
      e.stopPropagation();
      const newLang = currentLang === 'en' ? 'ru' : 'en';
      console.log('Language toggle touched, switching to:', newLang);
      updateLanguage(newLang);
    });
  }
